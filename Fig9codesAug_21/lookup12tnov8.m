   ...
   %  called from randPET3D.m
    ...
    function Hts = lookup12tnov8(phi)  % I passed syy1 syy2 as globals


% phi is scalar;  syy1,syy2 are the screen coords of the box of recon pts
% We TESTED already in onebeam that abs(phi) < PHIBMAX
 ...
 global syy1 ;  % the out-projected screen coords of the box of recon pts.
...
%   u33  = 1- sqrt(1-(phi/PHIBMAX)^2); % index in [0,1), % ERROR detected july3_21
 ...
 f33 = 1+f33 ;               % Thus f3+1 is in [2,Nphi]
    ...
    slice = (1-r33)*TBLK12t(:,:,f33) + r33*TBLK12t(:,:,1+f33);                ;
...
%    RG12 = 2.0*RTOF + 1.25*R12;  % from randPET...
     Hts = (0.5*(1-sign(-RG12+abs(syy0)))).*Hts;
