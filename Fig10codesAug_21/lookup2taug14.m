   ...
   %  called from randPET3D.m
    ...
    function Hts = lookup2taug14(phi)  % I passed syy1 syy2 as globals
...
% We TESTED already in onebeam that abs(phi) < PHIBMAX
   ...
   global TBLK1 TBLK1t TBLK2t
   global R12 R24 RTOF RG12 RG1t RG2t;
...
%   u33  = 1- sqrt(1-(phi/PHIBMAX)^2); % index in [0,1),  scalar ERROR july3_21
 ...
 f33 = 1+f33 ;               % Thus f3+1 is in [2,Nphi]
    ...
    slice = (1-r33)*TBLK2t(:,:,f33) + r33*TBLK2t(:,:,1+f33);                ;
...
%    RG12 = 2.0*RTOF + 1.25*R12; %  RG12 is cutoff for  f12t
% Hts = (0.5*(1-sign(-RG12+abs(syy0)))).*Hts;  %  ERROR aug13_20
% Hts = (0.5*(1-sign(-RG1t+abs(syy0)))).*Hts;  % cutoff for K1t and K2t
  Hts = (0.5*(1-sign(-RG2t+abs(syy0)))).*Hts;  % cutoff for K2t
