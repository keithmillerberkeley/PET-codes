   ...
   %  called from randPET3D.m
    ...
    function Hts = lookup1july5(phi)  % I passed syy1 syy2 as globals
...
% Recall y2 = 2*u2./(1-.998*u2) in mkTBLKPHI gives stretched y2 coord
       % thus u = y/(2+.998y) is the index in [0,1)
    u1 = syy1./(2+.998*syy1); % indices in [0,1) ; a matrix of size(syy1)
    u1 = (Ny1-1)*u1;  % indices in [0,Ny1-1)
    u2 = syy2./(2+.998*syy2); % indices in [0,1) ; a matrix of size(syy1)
    u2 = (Ny2-1)*u2;  % indices in [0,Ny2-1)

% Recall phi = PHIBMAX*(1-(1-u33)^2 )) in mkTBLKPHI gives stretched phi coord
    u33  = 1- sqrt(1-(phi/PHIBMAX)); % index in [0,1),  scalar  CORRECTED July3_21
%%%    u33  = 1- sqrt(1-(phi/PHIBMAX)^2); % index in [0,1),  scalar UNCORRECTED July3_21
 ...
 f33 = 1+f33 ;               % Thus f3+1 is in [2,Nphi]
    ...
    slice = (1-r33)*TBLK1(:,:,f33) + r33*TBLK1(:,:,1+f33);                ;
...
%     i +Ny1*(j-1) +Ny1*Ny2*(k-1) 
 
   u000 = f1 + Ny1*(f2-1);   % print
